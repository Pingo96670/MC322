...
import lab.sensors.WaterSensor;
import lab.entities.bots.BaseRobot;
import lab.entities.bots.interfaces.FluidHandler;
import lab.entities.bots.RobotState;
import lab.entities.bots.interfaces.WaterCarrier;
...
public class CamelRobot extends GroundRobot implements FluidHandler, WaterCarrier {
    ...
    public boolean isNearWater() throws RobotUnavailableException {
        ...
        EntityType[][][]matrix = BaseRobot.getEnvironment().getEntityTypeMatrix();

        // Checks for water in the orthogonally adjacent position (North, South, East, West, respectively)
        if((getPosZ()+1)<= BaseRobot.getEnvironment().getSizeZ() && matrix[getPosX()][getPosY()][getPosZ()+1]==EntityType.WATER) {
            return true;
        }
        ...
        if((getPosX()+1)<= BaseRobot.getEnvironment().getSizeX() && matrix[getPosX()+1][getPosY()][getPosZ()]==EntityType.WATER) {
            return true;
        }
    ...
    public void goToWater() throws RobotUnavailableException {
        ...
        if(waterLevel<storageCapacity) {
            ...
            int maxX = BaseRobot.getEnvironment().getSizeX();
            int maxZ = BaseRobot.getEnvironment().getSizeZ();
            EntityType[][][]matrix = BaseRobot.getEnvironment().getEntityTypeMatrix();
